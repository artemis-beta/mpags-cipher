...
PlayFairCipher::PlayFairCipher(const std::string &cipher_key) : key_{""}{ key_ = cipher_key;}
...
void PlayFairCipher::setKey(std::string key){
	...
	for(unsigned a{0}; a<key_.size(); ++a){key_[a]=toupper(key_[a]);}
	...
	for(unsigned k{0}; k<key_vec.size(); ++k){if(key_vec[k]=='J'){key_vec[k] = 'I';} std::cout << key_vec[k];}
...
key_vec.erase(std::remove(dup + 1, key_vec.end(), *dup), key_vec.end());
	...
	for(unsigned k{0}; k<key_vec.size(); ++k){ diff = std::remove(grid.begin(), grid.end(), key_vec[k]);}
	...
	grid.resize(26);
	for(unsigned k{0}; k<grid.size(); ++k){std::cout << grid[k] << std::endl;}

	// Store the coords for each letter
	// Store the playfair cipher key map

}
std::string PlayFairCipher::Encrypt(std::string &input_string){

inputstring_ = input_string;
...
std::string PlayFairCipher::Decrypt(std::string &input_string){
